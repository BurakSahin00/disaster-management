    ...
    <div className="bg-white rounded-2xl border border-border overflow-hidden">
      ...
      <div className="px-6 pt-6 pb-0">
        ...
        <div className="flex items-center gap-2 mt-2.5">
          <span className="text-[11px] text-text-faint">Hızlı seç:</span>
          {PRESETS.map((p) => (
            <button
              key={p.label}
              type="button"
      ...
      <div className="px-6 pb-6">
        <div className="grid grid-cols-2 gap-3 mb-5">
          <UploadZone label="Pre-Afet" sublabel="Afet öncesi görüntü" file={preFile} onFile={setPreFile} />
          <UploadZone label="Post-Afet" sublabel="Afet sonrası görüntü" file={postFile} onFile={setPostFile} />
        </div>
        ...
        >
          {loading ? 'Yükleniyor…' : ready ? 'Analizi Başlat →' : 'Pre ve post TIFF seçin'}
        </button>
      </div>
    </div>
  )

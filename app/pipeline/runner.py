...
class DamageAnalysisPipeline:
    ...
    def run(self, pre_path: str, post_path: str, output_dir: str = "outputs") -> dict:
        ...
        if polygons:
            ...
            # Tam görüntüleri yalnızca bina tespit edildiğinde belleğe al
            ...
            print("[3/3] Tespit edilen bina yok, sınıflandırma atlandı.")
            # post_img görsel çıktı için hâlâ gerekli
            post_img = self.loader.read_full(post_path)
            damage_labels = []
    ...
    def _save_outputs(
        ...
        # Görsel overlay — post_img (C, H, W) → BGR (H, W, 3) uint8

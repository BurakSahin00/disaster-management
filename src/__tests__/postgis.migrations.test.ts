...
describe('postgis migrations (integration)', () => {
  ...
  const shouldRun = hasRealDatabaseUrl(databaseUrl) && process.env.RUN_POSTGIS_MIGRATION_TESTS === '1';
  ...
  (shouldRun ? it : it.skip)('applies PostGIS + ERD migrations', async () => {
    ...
    for (const t of ['users', 'satellite_images', 'analyses', 'buildings', 'building_damages', 'clusters']) {
